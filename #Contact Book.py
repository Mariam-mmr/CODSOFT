...
class ContactBook:
    ...
    def view(self):
        #Making a check if the list is empty or not
        if len(self.contacts) == 0:  #or "if not self.contacts: "
            #Diaplay a guiding message to user
            print("The Contact Book List is empty")

        #if the list containts contacts and data
        else:
            #The iteration loop through all contacts
            for contact in self.contacts:
                #Printing contacts with their full details
    ...
    #"keyword" The parameter that the method accepts, which will be used to search for the contacts
    def srch(self, keyword):
        #Now making list comprehension for filtering contacts
        #"result" A list that will store the contacts that match the search
    ...
    #"nname=None, nphone=None,..." are optional parameters, if provided they will be used to update the corresponding fields...
    #...of the contact. if not provided, the fields will remain unchanged
    def updt(self, name, nname=None, nphone=None, nemail=None, naddress=None):
        ...
        for contact in self.contacts:
            ...
            #If the match is found, the method will update the contact's data
            if contact.name == name:
                #check if new name "nname" is provoded or not "None"
                if nname:
                    #if the new name "nname" is provided, the contact's "name" attribute will be updated to the new one
                    contact.name = nname
                #check if new phone "nphone" is provoded or not "None"
                if nphone:
                    #if the new phone "nphone" is provided, the contact's "phone" attribute will be updated to the new one
                    contact.phone = nphone
                #check if new email "nemail" is provoded or not "None"
                if nemail:
                    #if the new email "nemail" is provided, the contact's "email" attribute will be updated to the new one
                    contact.email = nemail
                #check if new address "naddress" is provoded or not "None"    
                if naddress:
                    #if the new address "naddress" is provided, the contact's "address" attribute will be updated to the new one
    ...
    def dlt(self, name):
        ...
        for contact in self.contacts:
            ...
            if contact.name == name:
                #if the match is found, then the method will delete the "contact" from the list "self.contacts"
    ...
    def usr_act(self):
        ...
        while True:
            ...
            #for choice 4, checking if user wants to update a contact
                ...
                nemail = input("Enter the new email (keep blank to keep current): ")
                nphone = input("Enter the new phone (keep blank to keep current): ")
                naddress = input("Enter the new address (keep blank to keep current): ")
                #Calling the method "updt" to update an existed contact
                self.updt(name, nname if nname else None, nphone if nphone else None,
                           nemail if nemail else None, naddress if naddress else None)
